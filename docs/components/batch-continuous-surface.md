...
```text
导入 PDF
  -> 在任意代表页绘制一次文档裁剪框，并可选择固定分栏数
  -> 裁剪框内显示按栏数均分的辅助线
  -> 按相同页面比例应用到本 PDF 的每一页
  -> 切换任意页并直接检查裁剪与分栏结果
  -> 确认裁剪与分栏（此后不可修改）
  -> 在单轨、等宽、首尾无缝的连续阅读单元带框题
...
- 禁止任何两个真实 PDF 页面并排显示。
- 固定分栏启用后，每个真实页按“页 × 栏”展开为连续阅读单元；单元内部可以显示当前栏及相邻半栏上下文，但仍只占连续带中的一个纵向位置。
...
- 分栏数在第一次裁剪页眉页脚时选择；`1` 表示不分栏。
- 分栏辅助线在裁剪框内按当前栏数均分，不提供逐页调整入口。
- 首次裁剪框使用细连续边框和明显更粗的八向手柄，PDF 编辑器外侧不绘制额外黑色边框。
- 当前版本的分栏配置对整份 PDF 生效。几何层通过按页解析布局的边界获取配置，为未来单页取消或修改分栏预留扩展点。
- 当前栏是所有阅读单元的横向对齐基准。当前栏左侧显示左邻栏的右半栏，右侧显示右邻栏的左半栏；存在的相邻半栏使用无色透明度渐变与向外增强的高斯模糊，不叠加黑色或其他色彩、不显示硬边框，鼠标进入该半栏时临时隐藏遮罩；边缘栏缺失的一侧显示为透明画布且不可框选，不得拉伸当前栏补满。
- PDF 暗色预览开关从首次裁剪步骤开始可用，并同时作用于裁剪、分栏预览和正式框题视图；邻栏遮罩必须针对 PDF 明暗模式分别保证足够对比度。
- 相邻阅读单元的可选源区域允许重叠，以容纳跨栏的手写作答；逻辑阅读顺序仍由均分后的当前栏决定。
- 分栏后的阅读顺序为：本页第 1 栏、本页第 2 栏……下一页第 1 栏。
- 本期不实现任何单页分栏覆盖 UI 或逐页布局持久化。
...
- 选框按文档位置排序，不按创建时间排序。
- 主排序键为阅读单元顺序（真实页、栏序号），再按栏内顶部 document-space 坐标。
- 顶部相同时按左侧坐标排序。
- 未提交选框的题号随排序更新。
- 已提交题号锁定，避免任务编号变化。
...
- 每个 part 保存 `column_index`；同一真实页可有多个有序 part，且源坐标允许重叠。
- 不再把旧 `continuation` 作为新交互事实。

### 10.2 自动保存

- 自动保存以裁剪、固定分栏、排除页和选框的规范化内容快照判断脏状态；恢复、滚动、切页、缩放、明暗切换和遮罩悬停不得触发保存。
- 前端必须用后端返回的规范化快照反校验保存结果；分栏或其他字段被旧后端丢弃时显示失败并重试，不得误报“已保存”。
- 保存成功后必须同步更新当前会话与最近文件缓存，最近文件明确显示持久化的栏数。
...
- 页面删除只把真实页索引写入 `excluded_page_indices`，不得改写或覆盖原 PDF。
- 被删除页从连续阅读单元带中排除，但左侧页栏保留原始页码和“恢复页面”入口。
- 至少保留一页；已有已提交题目的页面禁止删除。
- 删除立即执行，不显示确认框；删除页涉及的待提交选框整题删除，并以非阻塞通知说明，不得只移除其中一个 part。
- 删除或恢复页面后，其他选框必须用原 `parts[]` 在新阅读单元几何上重新投影，原始页码不重新编号。
- 如果恢复页面会插入到一个已有跨页选框中间，则已提交选框阻止恢复；待提交选框经确认后删除。
...
- 确认前可调整，确认后不可调整。
- 裁剪完成后恢复 session 一致。
- 删除和恢复指定页后，连续表面、原始页码与其他页面选框保持一致。
- 单列页面等宽并首尾无缝。
- 自动滚动向上、向下跨页创建一个选框。
- 跨 2 页和 3 页选框边界连续。
...
2. 一次裁剪和固定分栏按比例作用于当前 PDF 全部页面，确认后不可修改；本期没有逐页调整入口。
3. 裁剪阶段选择栏数时立即显示均分辅助线；预览可切换任意真实页检查展开结果。
4. 页面按“页 × 栏”展开为单轨、等宽、无缝的阅读单元，当前栏始终横向对齐，边缘缺失邻域不拉伸。
5. 跨页和跨栏选区使用同一个可八方向缩放的 `DocumentRect`。
6. 侧栏、缩放、滚动和懒加载不造成选区坐标漂移。
7. `parts[]` 支持任意页数与同页多栏，保存 `column_index`，允许源坐标重叠并兼容读取旧 session。
