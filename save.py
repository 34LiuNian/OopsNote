...
class MongoSaver:
    ...
    def _connect(self):
        ...
        try:
            ...
            logger.info(f"成功连接到 MongoDB: {self.mongo_uri}, 数据库: {self.database_name}, 集合: {self.collection_name}")
        except ConnectionFailure:
            logger.error(f"无法连接到 MongoDB 服务器 at {self.mongo_uri}")
            self.client = None # 连接失败，重置 client
            raise ConnectionFailure(f"无法连接到 MongoDB 服务器 at {self.mongo_uri}")
        except Exception as e:
            logger.error(f"连接 MongoDB 时发生意外错误: {e}")
            self.client = None
            raise e # 重新抛出异常
    ...
    def save_oops(self, oops_instance: Oops) -> Optional[str]:
        ...
        if self.client is None or self.collection is None:
            logger.error("MongoDB 连接未建立，无法保存。")
            return None
        ...
        try:
            ...
            logger.info(f"成功将 Oops 对象写入 MongoDB: {inserted_id}")
            logger.debug(f"写入的文档内容: {oops_dict}")

            return inserted_id

        except OperationFailure as e:
            logger.error(f"MongoDB 操作失败: {e.details}")
            return None
        except Exception as e:
            logger.error(f"保存到 MongoDB 时发生意外错误: {e}")
            return None
    ...
    def _verify_write(self, inserted_id_str: str):
        ...
        try:
            ...
            if retrieved_doc:
                logger.debug(f"检索到的文档: {retrieved_doc}")
                # 验证 Pydantic 模型是否能解析检索到的文档
                retrieved_oops = Oops.model_validate(retrieved_doc)
                print(retrieved_oops)
            else:
                logger.error("未能检索到刚写入的文档。")
        except ImportError:
             logger.error("验证失败：bson 模块未找到。")
        except Exception as e:
             logger.error(f"验证写入时出错: {e}")
