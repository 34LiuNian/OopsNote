          ...
          <Box sx={{ p: 3, border: "1px solid", borderColor: "border.default", borderRadius: 2 }}>
            ...
            <Box sx={{ mt: 3 }}>
              <FormControl>
                ...
                <TextInput value={newAliases} onChange={(e) => setNewAliases(e.target.value)} placeholder="同义词 1, 同义词 2" block />
              </FormControl>
            </Box>
      ...
      <Box sx={{ p: 3, border: "1px solid", borderColor: "border.default", borderRadius: 2 }}>
        ...
        <Box sx={{ mt: 4, display: "flex", flexDirection: "column", gap: 4 }}>
          {TAG_DIMENSIONS.map((dim) => {
            const items = allItems.filter((t) => t.dimension === dim.key);
            if (items.length === 0) return null;
            return (
              <Box key={dim.key}>
                <Heading as="h4" sx={{ fontSize: 2, mb: 2, borderBottom: "1px solid", borderColor: "border.default", pb: 1 }}>
                  {effectiveDims[dim.key]?.label || dim.fallbackLabel}
                  <Text sx={{ ml: 2, color: "fg.muted", fontSize: 1 }}>{items.length}</Text>
                </Heading>
