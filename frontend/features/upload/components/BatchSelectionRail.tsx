...
import { Box, GeometryButton, IconButton, NativeSelect, Text } from "@/components/ui/primitives";
      ...
      <div className="batch-selection-list">
        {selections.map((selection) => (
          <div key={selection.id} className={`batch-selection-list__item${selection.id === activeSelectionId ? " is-active" : ""}`}>
            <GeometryButton type="button" className="batch-selection-list__primary" onClick={() => onActivate(selection)}>
            ...
            <label className="batch-selection-list__review" title="标记或取消人工复核">
              ...
              <NativeSelect aria-label={`第 ${selection.questionNo} 题异常状态`} value={selection.reviewReason ?? ""} onChange={(event) => onReview(selection, event.target.value as SelectionReviewReason | "")}>
