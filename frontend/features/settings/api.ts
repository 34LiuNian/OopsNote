...
export function rotateAiCredential(profileId: string, secret: string) {
  return fetchJson<{ profile: AiProviderProfile }>(`/settings/provider-profiles/${encodeURIComponent(profileId)}/secret`, { method: "POST", body: JSON.stringify({ secret }) });
}
