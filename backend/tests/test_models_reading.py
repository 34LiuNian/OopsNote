...
def _make_client(
    monkeypatch, bootstrap_module, main_module, *, startup_fetch: bool
) -> TestClient:
...
def test_models_uses_cache_when_present(monkeypatch):
    ...
    client = _make_client(
        monkeypatch, bootstrap_module, main_module, startup_fetch=False
    )
    ...
    def _should_not_fetch(*args: Any, **kwargs: Any):
        raise AssertionError(
            "_fetch_openai_models should not be called when cache exists"
        )

    monkeypatch.setattr(
        bootstrap_module, "_fetch_openai_models", _should_not_fetch, raising=True
    )
...
def test_models_refresh_true_fetches(monkeypatch):
    ...
    monkeypatch.setattr(
        bootstrap_module,
        "_guess_openai_gateway_config",
        lambda: ("http://gw/v1", None, "Bearer t", "Authorization"),
        raising=True,
    )
    monkeypatch.setattr(
        bootstrap_module, "_fetch_openai_models", _fake_fetch, raising=True
    )
